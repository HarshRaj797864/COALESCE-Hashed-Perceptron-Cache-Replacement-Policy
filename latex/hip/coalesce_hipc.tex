...
The MPKI column of Table~\ref{tab:ocean} carries two observations. First, LRU attains the \emph{lowest} miss count at both scales (74.0 and 36.7 MPKI) yet finishes 6--7\% behind SHiP: on a coherent LLC, miss counts and performance can invert, because LRU's recency bias retains dirty lines whose eventual misses are cheap while evicting lines whose refetches sit on the critical path. Second, Mockingjay's degradation is a genuine miss explosion---146.1 MPKI, $1.7\times$ SHiP's---not a latency artefact, which further argues against an implementation-fidelity explanation.
