...
\label{eq:bias}
\end{equation}
The gating prevents the bias from rescuing blocks the perceptron has confidently learned are dead, and makes the policy insensitive to the exact magnitudes: the bias only reorders blocks the perceptron already favours. The magnitudes themselves were set empirically from a configuration sweep on 8-core canneal over $(B_M, B_S) \in \{(0,0),\,(0,20),\,(40,0),\,(150,75)\}$ relative to the chosen $(40, 20)$: the chosen configuration finishes \todo{X}\% ahead of the bias-free variant $(0,0)$ and \todo{Y}\% ahead of the over-biased $(150,75)$, with the single-component variants in between, confirming that a moderate gated bias helps and that the policy is not hypersensitive to its exact magnitude. The victim is the way with the \emph{lowest} final vote (Algorithm~\ref{alg:victim}).
...
\begin{table}[htbp]
    ...
    \begin{tabular}{ll}
        ...
        Prefetcher & none \\
        Warmup / simulation & 50\,M / 100\,M instructions per core\textsuperscript{$\dagger$} \\
...
The win concentrates exactly where it gates performance. canneal's execution time is set by its two bottleneck cores, and COALESCE serves them best: at 8 cores its bottleneck-core IPCs are 0.333/0.331, against 0.331/0.330 for SRRIP, 0.317/0.316 for SHiP, 0.315/0.316 for Hawkeye, and ${\sim}0.255$ for both Mockingjay and LRU---a 5.5\% lift over Hawkeye and 30\% over Mockingjay on precisely the cores that determine total cycles.

At 16 cores under the overlay, COALESCE completes the same per-core work in 348.2\,M cycles, with bottleneck-core IPC declining only modestly (0.374 at 4 cores, 0.332 at 8, 0.294 at 16) as per-core effective LLC capacity shrinks by $4\times$ (Figure~\ref{fig:scaling}). We focus per-policy comparisons at 4 and 8 cores and defer full 16-core baselines to an extended evaluation; the scaling trend indicates that per-core effective LLC capacity is the dominant variable, so the qualitative ranking at 16 cores is expected to track the 8-core ordering.
...
Across all four workloads a consistent pattern emerges for the strongest multi-programmed policies (Figure~\ref{fig:geomean}). Mockingjay---which improves on LRU by 15.2\% averaged over one hundred multi-programmed mixes in its original evaluation~\cite{shah2022mockingjay}---finishes \emph{last on three of the four workloads in this study} and second-to-last among the baselines on the fourth: 30.5\% behind COALESCE on canneal at 8 cores, 48\% behind SRRIP on ocean at both scales, and behind LRU on fluidanimate and barnes. Hawkeye fares better but still loses to LRU on ocean and barnes. We hypothesise distribution shift as the cause. Mockingjay's reuse-distance predictor is trained online from PC-indexed samples that, under multithreaded sharing, conflate accesses from different cores to the same lines. Invalidations also remove blocks at times its estimated-time-remaining model cannot anticipate. Hawkeye's milder degradation is consistent with the same account: its binary cache-friendly/cache-averse labels also lose meaning when one PC touches both private and shared lines, but a binary label bounds---without eliminating---the damage of any single misprediction, whereas an erroneous fine-grained reuse distance can misorder an entire set. Confirming this account requires predictor-accuracy instrumentation on shared versus private lines, which we leave to future work. What the data already establishes is the headline: a policy that approaches MIN on the distribution it was designed for can sit below LRU outside it. We believe this is the paper's most consequential observation for the field: \emph{evaluation methodology, not predictor capacity, is the current bottleneck for learned replacement on multithreaded workloads.}

\subsection{Suite-wide summary}

Figure~\ref{fig:geomean} reports geometric-mean speedups of COALESCE over each baseline, losses included. Across the four-workload suite at 4 cores---the paper's summary statistic---the geomean speedup over LRU is $+9.7\%$ for COALESCE, $+8.1\%$ for Hawkeye, and $-3.7\%$ for Mockingjay; against the RRIP family COALESCE sits between $+0.2\%$ and $-0.5\%$. At 8 cores, only three workloads have 8-core data (canneal, ocean, fluidanimate; barnes was run at 4 cores), and on that reduced set COALESCE achieves $+9.2\%$ over LRU, $+1.5\%$ over Hawkeye, and $+22.5\%$ over Mockingjay, with $-1.0\%$ to $-2.4\%$ against SHiP and SRRIP. We emphasise that the two scales are not directly comparable---the 8-core set excludes barnes, a workload where COALESCE trails---which is why we treat the 4-core suite as the headline summary. The overall picture is fair to state plainly: COALESCE wins decisively inside its target class, concedes modest ground to RRIP heuristics outside it, and is the strongest learning-based policy at both scales.
...
ChampSim is deterministic for a fixed configuration, so the relevant variation is across configuration perturbations rather than repeated runs. We quantify it by re-running an 8-core canneal configuration under three virtual-memory randomisation seeds---which perturb physical-page assignment and therefore set-index conflict patterns---for the policies closest to the headline comparison, holding the workload mix and every other parameter fixed. This is a separate variance experiment on its own fixed mix, not error bars on Table~\ref{tab:canneal}.

\begin{table}[htbp]
    \centering
    \renewcommand{\arraystretch}{1.15}
    \caption{Seed sensitivity: total cycles across three VMEM randomisation seeds (8-core canneal, fixed mix)}
    \label{tab:variance}
    \resizebox{0.9\linewidth}{!}{%
    \begin{tabular}{lrrr}
        \toprule
        \textbf{Policy} & \textbf{min} & \textbf{max} & range/mean \\
        \midrule
        COALESCE & \todo{--} & \todo{--} & \todo{--} \\
        SRRIP & \todo{--} & \todo{--} & \todo{--} \\
        SHiP & \todo{--} & \todo{--} & \todo{--} \\
        Hawkeye & \todo{--} & \todo{--} & \todo{--} \\
        \bottomrule
...
Across seeds, the per-policy spread in total cycles is \todo{X}\% of the mean or less, and the policy ranking is \todo{unchanged} in every seed. The 0.4\% COALESCE--SRRIP gap of Section~\ref{sec:canneal} is \todo{within / outside} this band, \todo{consistent with the tie treatment adopted there / indicating the margin is robust to page-mapping perturbation}; the 5\% margin over Hawkeye and the ${\sim}30\%$ margins over Mockingjay and LRU exceed the band by an order of magnitude at least.

\section{Feature Ablation: When Does the Sharer Count Matter?}
\label{sec:ablation}

A multi-feature policy must show that each of its features earns its storage. We ablate COALESCE's sharer-count axis: the ablated variant (\emph{no-sharer}) replaces $h_1$'s sharer input with a second PC-only hash (a distinct mixing constant, after~\cite{jimenez2017multiperspective}) and removes the $+20\times$sharers bias, leaving the MESI axis and all other machinery intact. Figure~\ref{fig:ablation} and Table~\ref{tab:ablation} compare the two on every workload.

fluidanimate is omitted from Table~\ref{tab:ablation} by construction: under its 100\% read traffic the sharer count never exceeds one, the ablated input carries no information, and the comparison reduces to hash-mixing noise.
...
\textbf{Learned replacement.} Sampling dead-block prediction~\cite{khan2010sdbp} and reuse-distance--based replacement~\cite{keramidas2007rdip} established the prediction framing that the modern line of work builds on. Hawkeye~\cite{jain2016hawkeye} casts replacement as supervised imitation of Belady's MIN~\cite{belady1966study}; Mockingjay~\cite{shah2022mockingjay} sharpens the target from binary labels to reuse distances; Glider~\cite{shi2019glider} distils offline deep models into deployable predictors; multiperspective reuse prediction~\cite{jimenez2017multiperspective} and perceptron-based reuse prediction bring the hashed-perceptron idiom~\cite{jimenez2001perceptron} to caches, which COALESCE adopts. Cost-conscious learned policies have also been designed with reinforcement learning~\cite{sethumurugan2021designing,souza2024rl} and concurrency-aware perceptrons for GPU-class miss-cost prediction~\cite{wu2025camp}; of these, Souza and Freitas~\cite{souza2024rl} is the closest prior work to our setting, targeting multicore processors with reinforcement learning, though without coherence-state features and without multithreaded-sharing evaluation. All of these evaluate on single-threaded or multi-programmed workloads; none observe coherence state. To our knowledge COALESCE is the first replacement policy to use MESI state and sharer count as learned features, and this paper is the first head-to-head evaluation of learned replacement under genuine multithreaded sharing.
