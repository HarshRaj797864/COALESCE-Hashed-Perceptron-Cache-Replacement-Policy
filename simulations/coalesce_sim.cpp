...
public:
    ...
    void train(uint64_t pc, int sharers, bool positive) {
        ...
        if (positive) {
            if (table0[idx0] < 127) table0[idx0]++;
            if (table1[idx1] < 127) table1[idx1]++;
        } else {
            if (table0[idx0] > -128) table0[idx0]--;
            if (table1[idx1] > -128) table1[idx1]--;
        }
    }
...
public:
    CacheSet(PerceptronPredictor *p) : brain(p) {
        ...
        for(int i=0; i<CACHE_WAYS; i++) ways[i].lru_stack = i;
    }
    ...
    int access(uint64_t address, uint64_t pc, int sharers) {
        ...
        // Replace
        ways[victim_way] = {true, address, pc, sharers, 0}; // Becomes MRU
    ...
    int find_victim_way() {
        ...
        for (int i = 0; i < CACHE_WAYS; i++) {
            if (!ways[i].valid) return i; // Empty slot
            if (ways[i].lru_stack == CACHE_WAYS - 1) lru_victim = i;
        ...
        if (worst_vote > 0) return lru_victim;
...
int main() {
    ...
    for (int epoch = 0; epoch < 10; epoch++) {
        for (int i = 0; i < 200; i++) {
            ...
            if (res1 == 1) hits++; else if (res1 == 2) bypasses++; else misses++;
            ...
            if (res2 == 1) hits++; else if (res2 == 2) bypasses++; else misses++;
        }
    }
