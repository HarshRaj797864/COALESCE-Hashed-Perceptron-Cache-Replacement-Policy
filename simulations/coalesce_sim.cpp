...
public:
    ...
    void update_on_hit(int set_idx, int way, const CacheLine &line) override
    {
        ...
        for (int w = 0; w < WAYS; w++)
        {
            if (lru_stacks[set_idx][w] < old_pos)
                lru_stacks[set_idx][w]++;
        }
    ...
    int find_victim(int set_idx, const std::vector<CacheLine> &set, uint64_t pc, int sh, MESI_State st) override
    {
        for (int w = 0; w < WAYS; w++)
        {
            if (!set[w].valid)
                return w;
            if (lru_stacks[set_idx][w] == WAYS - 1)
                return w;
        }
        return 0;
    }
...
public:
    COALESCE_Policy(PerceptronBrain *b, int rate = SAMPLING_RATE) : brain(b)
    {
...
int main()
{
    ...
    // SCENARIO 2: Hot-Set contention (Coherence Critical)
    ...
    auto run_test = [&](ReplacementPolicy *p)
    {
        ...
        for (int epoch = 0; epoch < 10; epoch++)
        {
            ...
            // Phase B: The Ping-Pong (PC 0xF00D) - High Coherence (Sharers=4, MODIFIED)
            for (int k = 0; k < 400; k++)
            {
    ...
    run_test(&coalesce);
    // --- EXPERIMENT 2: Epoch Learning Curve ---
    std::cout << "\n[Experiment 2] COALESCE Learning Progress" << std::endl;
    for (int ep : {1, 5, 20, 50})
    {
        PerceptronBrain b_ep;
        COALESCE_Policy p_ep(&b_ep, 32);
        Simulator s_ep(&p_ep);
        for (int i = 0; i < ep * 500; i++)
        {
            s_ep.access(1000 + (i % 600), 0xBAD, 0, EXCLUSIVE); // Scanner
            s_ep.access(i % 16, 0xF00D, 4, MODIFIED);           // Hot Set
        }
        std::cout << "Epochs: " << ep << " | Hit Rate: " << (100.0 * s_ep.hits / (s_ep.hits + s_ep.misses)) << "%" << std::endl;
    }
